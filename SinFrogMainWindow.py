...
class SinFrogMainWindow(wx.Frame):
    ...
    def saveMenu(self, event):  # wxGlade: SinFrogMainWindow.<event_handler>
        if (self.DistrictsCombobox.GetCurrentSelection() > -1) and (self.IspCombobox.GetCurrentSelection() > -1) and (self.PlanCombobox.GetCurrentSelection() > -1):
            ...
            self.SinFrogMainWindow_statusbar.SetStatusText("ERROR: Please select both distric, IPS and plan name.")
    ...
    def runButton(self, event):  # wxGlade: SinFrogMainWindow.<event_handler>
        ...
        verbose_ping("google.fr", displayTarget = display)
        display.DisplayFrame_statusbar.SetStatusText("Pinging done")
        
        display.DisplayFrame_statusbar.SetStatusText("Speed testing")
        display.DisplayCtrl.AppendText("\n" + "Downloading all file in one trunk")
        t = SpeedClass('http://itc.conversationsnetwork.org/audio/download/ITC.SO-Episode69-2009.09.29.mp3', display)
        t.start()
        display.DisplayFrame_statusbar.SetStatusText("Speed testing done")
        
        display.DisplayFrame_statusbar.SetStatusText("Please close this window manually for the demo")
